/**
 * Source: https://svn.apache.org/repos/asf/activemq/trunk/activemq-kahadb-store/src/main/java/org/apache/activemq/util/LFUCache.java
 * 
 */
...
public class LFUCache<Key, Value> implements Map<Key, Value> {
	...
	public LFUCache(int maxCacheSize, float evictionFactor) {
		if(evictionFactor <= 0 || evictionFactor >=1) {
			throw new IllegalArgumentException("Eviction factor must be "
	...
	public Collection<Value> values() {
		return (Collection<Value>) this.cache.values();
	}
	...
	private void doEviction() {
		...
		while(currentlyDeleted < target) {
			...
			if(nodes.isEmpty()) {
				throw new IllegalStateException("Lowest freqency constraint violated.");
			} else {
